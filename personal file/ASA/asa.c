...
#define ASA_ID "/* $Id: asa.c,v 30.3 2014/01/06 20:29:56 ingber Exp ingber $ */"
...
#endif /* HAVE_ANSI */
{
  ...
  *curvature_flag = FALSE;
  ...
  if ((save_queue_flag_par =
       (int **) calloc (OPTIONS->Gener_Block_Max, sizeof (int *))) == NULL) {
    strcpy (exit_msg, "asa(): *save_queue_flag_par");
    Exit_ASA (exit_msg);
    *exit_status = CALLOC_FAILED;
    return (-1);
  }
  ...
  for (i_prll = 0; i_prll < OPTIONS->Gener_Block_Max; ++i_prll) {
    if ((save_queue_flag_par[i_prll] =
         (int *) calloc (queue_size_tmp, sizeof (int))) == NULL) {
      strcpy (exit_msg, "asa(): save_queue_flag_par[i_prll]");
      Exit_ASA (exit_msg);
      *exit_status = CALLOC_FAILED;
      return (-1);
    }
  ...
  for (i_prll = 0; i_prll < OPTIONS->Gener_Block_Max; ++i_prll) {
    for (queue = 0; queue < (LONG_INT) queue_size_tmp; ++queue) {
      VFOR (index_v) {
        ...
        save_queue_param_par[i_prll][queue_v] =
          current_generated_state->parameter[index_v];
      }
      save_queue_cost_par[i_prll][queue] = current_generated_state->cost;
      save_queue_flag_par[i_prll][queue] = *valid_state_generated_flag;
    }
    ...
    for (i_prll = 0; i_prll < OPTIONS->Gener_Block; ++i_prll) {
      ...
      for (queue = 0; queue < (LONG_INT) queue_size_tmp; ++queue) {
        VFOR (index_v) {
          ...
          queue_v = index_v + queue * (LONG_INT) (*number_parameters);
          save_queue_param_par[i_prll][queue_v] = save_queue_param[queue_v] =
            current_generated_state->parameter[index_v];
        }
        save_queue_cost_par[i_prll][queue_v] = save_queue_cost[queue] =
          current_generated_state->cost;
        save_queue_flag_par[i_prll][queue_v] = save_queue_flag[queue] =
          *valid_state_generated_flag;
      }
      save_queue_par[i_prll] = save_queue_indx_par[i_prll] = 0;
#endif /* ASA_QUEUE */
...
#ifdef _OPENMP
    omp_set_num_threads ((int) OPTIONS->Gener_Block);   /* requests threads */
#if ASA_QUEUE
#pragma omp parallel for private(index_v, queue)
#else
#pragma omp parallel for
#endif
#endif /* _OPENMP */
    for (i_prll = 0; i_prll < OPTIONS->Gener_Block; ++i_prll) {
      repeated_invalid_states_par[i_prll] = 0;
      do {
        ++number_invalid_generated_states_par[i_prll];
        generate_flg_par[i_prll] = generate_new_state (user_random_generator,
                                                       seed,
                                                       parameter_minimum,
                                                       parameter_maximum,
                                                       current_user_parameter_temp,
        ...
        if (OPTIONS->Queue_Size == 0) {
          ...
          queue_new_par[i_prll] = 1;
          for (queue = 0; queue < save_queue_par[i_prll]; ++queue) {
            ...
            if (save_queue_test_par[i_prll] == *number_parameters) {
              tmp_var_db_par[i_prll] = save_queue_cost_par[i_prll][queue];
              queue_new_par[i_prll] = 0;
              valid_state_generated_flag_par[i_prll] =
                save_queue_flag_par[i_prll][queue];
              if (valid_state_generated_flag_par[i_prll] == FALSE) {
#if ASA_PRINT_MORE
#if INT_LONG
                fprintf_flg_par[i_prll] = fprintf (ptr_asa_out,
        ...
        if (queue_new_par[i_prll] == 1) {
          ...
          tmp_var_db1_par[i_prll] =
            cost_function_test (tmp_var_db_par[i_prll],
                                current_generated_state->parameter,
          ...
          if (OPTIONS->Queue_Size > 0) {        /* in case recursive use */
            ...
            save_queue_cost_par[i_prll][save_queue_indx_par[i_prll]] =
              tmp_var_db_par[i_prll];
            save_queue_flag_par[i_prll][save_queue_indx_par[i_prll]]
              = valid_state_generated_flag_par[i_prll];
        ...
        tmp_var_db_par[i_prll] =
          user_cost_function (current_generated_state->parameter,
                              ...
                              OPTIONS);
        tmp_var_db1_par[i_prll] = cost_function_test
          (tmp_var_db_par[i_prll], current_generated_state->parameter,
           parameter_minimum, parameter_maximum, number_parameters,
           xnumber_parameters);
        if (tmp_var_db1_par[i_prll] == 0) {
          EXIT_asa_parallel = 3;
...
#endif /* ASA_QUEUE */
        ++repeated_invalid_states_par[i_prll];
        if (repeated_invalid_states_par[i_prll] >
            OPTIONS->Limit_Invalid_Generated_States) {
          EXIT_asa_parallel = 4;
        }
      }
      while (valid_state_generated_flag_par[i_prll] == FALSE);
      --number_invalid_generated_states_par[i_prll];
      gener_block_state[i_prll].cost = tmp_var_db_par[i_prll];
#if USER_ACCEPTANCE_TEST
      gener_block_state[i_prll].par_user_accept_flag =
    ...
    if (EXIT_asa_parallel == 1) {
      *exit_status = IMMEDIATE_EXIT;
      goto EXIT_asa;
    } else if (EXIT_asa_parallel == 2) {
      *exit_status = INVALID_COST_FUNCTION;
      goto EXIT_asa;
    } else if (EXIT_asa_parallel == 3) {
      *exit_status = INVALID_COST_FUNCTION;
      goto EXIT_asa;
    } else if (EXIT_asa_parallel == 4) {
      *exit_status = TOO_MANY_INVALID_STATES;
      goto EXIT_asa;
    }
#else /* ASA_PARALLEL */
...
#if ASA_PARALLEL
        /* leave i_prll loop after new minimum
           to start new generated states from this new minimum */
        break;
#endif /* ASA_PARALLEL */
      }
#if ASA_PARALLEL
...
#if ASA_QUEUE
#if 0                           /* can exit anyway */
  for (i_prll = 0; i_prll < OPTIONS->Gener_Block_Max; ++i_prll) {
    free (save_queue_param_par[i_prll]);
    free (save_queue_flag_par[i_prll]);
    free (save_queue_cost_par[i_prll]);
  }
  free (save_queue_flag_par);
